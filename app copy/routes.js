...
// normal routes ===============================================================
    ...
    app.get('/owned', isLoggedIn, function(req, res) {
        db.collection('cards').find({"user": req.session.passport.user}).toArray((err, results) => {
...
// message board routes ===============================================================
  //restart the server each time I make a update
    app.post('/magic', (req, res) => {
      db.collection('cards').save({user: req.session.passport.user, url: req.body.url}, (err, result) => {
        if (err) return console.log(err)
